...
public class GeneralPurposeController {
	...
	@RequestMapping(value = "/generateContentTillQual4", method = RequestMethod.GET)
	public @ResponseBody String generateContentTillQual4(@RequestParam(name= "companyId") String companyId,@RequestParam(name= "pathtoFile") String pathtoFile, HttpServletRequest request, HttpServletResponse response) throws Exception{
		...
		List<String> lines = FileUtils.readLines(new File(pathtoFile));
			for(String line : lines){
				...
				List<Question> qs = questionRep.findQuestionsByQualifier4(companyId, q1, q2, q3, q4);
				System.out.println(q1+" "+q2+" "+q3+" "+q4 +" size "+qs.size());
				Company company = companyService.findByCompanyId(companyId);
					if(qs.size() > 16){
						if(qs.size() <=100){
							//1 batch for a set of 5 ass
							System.out.println("batch 1 processing");
							processsQuestions(qs, q1, q2, q3, q4, null, company.getCompanyId(), company.getCompanyName(), "");
							System.out.println("batch 1 processing end");
						}
						else if (qs.size()> 100 && qs.size() <= 200){
							//2 batch for a set of 5 ass
							System.out.println("batch 2 processing");
							List<List<Question>> batches = getBatches(qs, 2);
								if(batches.size() != 2){
							...
							for(List<Question> qsBatch : batches){
								count ++;
								processsQuestions(qsBatch, q1, q2, q3, q4, null, company.getCompanyId(), company.getCompanyName(), "Part "+count);
							}
							System.out.println("batch 2 processing end");
						}
						else if (qs.size()> 200 && qs.size() <= 300){
							//3 batch for a set of 5 ass
							System.out.println("batch 3 processing");
							List<List<Question>> batches =  getBatches(qs, 3);
							if(batches.size() != 3){
								throw new AssessmentGenericException("WRONG_BATCH_SIZE");
							}
							int count = 0;
							...
							System.out.println("batch 3 processing end");
						}
						else if (qs.size()> 300 && qs.size() <= 400){
							//4 batch for a set of 5 ass
							System.out.println("batch 4 processing");
							List<List<Question>> batches =  getBatches(qs, 4);
							if(batches.size() != 4){
								throw new AssessmentGenericException("WRONG_BATCH_SIZE");
							}
							int count = 0;
							...
							System.out.println("batch 4 processing end");
						}
						else if (qs.size()> 400 && qs.size() <= 500){
							//5 batch for a set of 5 ass
							System.out.println("batch 5 processing");
							List<List<Question>> batches =  getBatches(qs, 5);
							if(batches.size() != 5){
								throw new AssessmentGenericException("WRONG_BATCH_SIZE");
							}
							int count = 0;
							...
							System.out.println("batch 5 processing end");
						}
						else if (qs.size()> 500 && qs.size() <= 600){
							//6 batch for a set of 5 ass
							System.out.println("batch 6 processing");
							List<List<Question>> batches =  getBatches(qs, 6);
							if(batches.size() != 6){
								throw new AssessmentGenericException("WRONG_BATCH_SIZE");
							}
							int count = 0;
							...
							System.out.println("batch 6 processing end");
						}
						else if (qs.size()> 600 && qs.size() <= 700){
							//7 batch for a set of 5 ass
							System.out.println("batch 7 processing");
							List<List<Question>> batches =  getBatches(qs, 7);
							if(batches.size() != 7){
								throw new AssessmentGenericException("WRONG_BATCH_SIZE");
							}
							int count = 0;
							...
							System.out.println("batch 7 processing end");
						}
						else if (qs.size()> 700 && qs.size() <= 800){
							//8 batch for a set of 5 ass
							System.out.println("batch 8 processing");
							List<List<Question>> batches =  getBatches(qs, 8);
							if(batches.size() != 8){
								throw new AssessmentGenericException("WRONG_BATCH_SIZE");
							}
							int count = 0;
							...
							System.out.println("batch 8  processing end");
						}
						else if (qs.size()> 800 && qs.size() <= 900){
							//9 batch for a set of 5 ass
							System.out.println("batch 9 processing");
							List<List<Question>> batches =  getBatches(qs, 9);
							if(batches.size() != 9){
								throw new AssessmentGenericException("WRONG_BATCH_SIZE");
							}
							int count = 0;
							...
							System.out.println("batch 9 processing end");
						}
						else if (qs.size()> 900 && qs.size() <= 1000){
							//10 batch for a set of 5 ass
							System.out.println("batch 10 processing");
							List<List<Question>> batches =  getBatches(qs, 10);
							if(batches.size() != 10){
								throw new AssessmentGenericException("WRONG_BATCH_SIZE");
							}
							int count = 0;
							...
							System.out.println("batch 10 processing end");
						}
						else if (qs.size()> 1000 && qs.size() <= 1100){
							//11 batch for a set of 5 ass
							System.out.println("batch 11 processing");
							List<List<Question>> batches =  getBatches(qs, 11);
							if(batches.size() != 11){
								throw new AssessmentGenericException("WRONG_BATCH_SIZE");
							}
							int count = 0;
							...
							System.out.println("batch 11 processing end");
						}
						else{
							System.out.println("batch 12 processing");
							List<List<Question>> batches =  getBatches(qs, 12);
							if(batches.size() != 12){
								throw new AssessmentGenericException("WRONG_BATCH_SIZE");
							}
							int count = 0;
							...
							System.out.println("batch 12 processing end");
						}
					}
	...
	@RequestMapping(value = "/generateContentTillQual5", method = RequestMethod.GET)
	public @ResponseBody String generateContentTillQual5(@RequestParam(name= "companyId") String companyId,@RequestParam(name= "pathtoFile") String pathtoFile, HttpServletRequest request, HttpServletResponse response) throws Exception{
		...
		List<String> lines = FileUtils.readLines(new File(pathtoFile));
			for(String line : lines){
				...
				List<Question> qs = questionRep.findQuestionsByQualifier5(companyId, q1, q2, q3, q4, q5);
				System.out.println(q1+" "+q2+" "+q3+" "+q4 +" "+q5+"  size "+qs.size());
				Company company = companyService.findByCompanyId(companyId);
					if(qs.size() >= 15){
						if(qs.size() <=100){
							//1 batch for a set of 5 ass
							System.out.println("batch 1 processing");
							processsQuestions(qs, q1, q2, q3, q4, q5, company.getCompanyId(), company.getCompanyName(), "");
							System.out.println("batch 1 processing end");
						}
						else if (qs.size()> 100 && qs.size() <= 200){
							//2 batch for a set of 5 ass
							...
							for(List<Question> qsBatch : batches){
								count ++;
								processsQuestions(qsBatch, q1, q2, q3, q4, q5, company.getCompanyId(), company.getCompanyName(), "Part "+count);
							}
							System.out.println("batch 2 processing end");
						}
						else if (qs.size()> 200 && qs.size() <= 300){
	...
	private void processsQuestions(List<Question> qs, String q1, String q2, String q3, String q4, String q5, String companyId, String companyName, String alias) throws IOException{
		...
		tests.add(dto4);
		tests.add(dto5);
		NormalAdaptiveAssessmentDto dto = createAdaptiveAssessment(tests, companyId, companyName, alias);
		Company company = companyService.findByCompanyId(companyId);
		createAssUploadData(dto, company);
		FileUtils.write(new File("ass_gulrez2.txt"), q1+","+q2+","+q3+","+q4+" - "+q5+","+dto.getAdaptiveAssessmentName()+"\n", true);
	}
	...
	protected NormalAssessmentDto createNormalAssessment(String companyId, String companyName, String q1, String q2, String q3, String q4, String q5, List<Question> questions, String alias){
		...
		String testName = "Assessment - "+q1+"-"+q2+"-"+q3+"-"+q4+(q5==null?"":"-"+q5)+alias;
	...
	protected NormalAdaptiveAssessmentDto createAdaptiveAssessment(List<NormalAssessmentDto> tests, String companyId, String companyName, String alias){
		...
		name+= tests.get(0).getQ1()+"-"+tests.get(0).getQ2()+"-"+tests.get(0).getQ3()+"-"+tests.get(0).getQ4()+(tests.get(0).getQ5() == null?"":"-"+tests.get(0).getQ5()) + alias;
