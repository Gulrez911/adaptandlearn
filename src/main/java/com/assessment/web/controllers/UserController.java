...
import com.assessment.web.dto.LMSAdminDtO;
...
public class UserController {
	...
	@Autowired
	LMSUserModuleMappingRepository lmsUserModuleMappingRepository;
	...
	@ResponseBody
	public String uploadUsers(HttpServletResponse response, MultipartHttpServletRequest request) throws Exception {
		try {
...
//			    File file = new File("users.xml");
	...
	@RequestMapping(value = "/uploadUsersWithLicenses", method = RequestMethod.POST)
	public @ResponseBody String uploadUsersWithLicenses(HttpServletResponse response, MultipartHttpServletRequest request) throws Exception {
		try {
			// System.out.println("in uploadUsers entering");
			User user = (User) request.getSession().getAttribute("user");
			MultipartFile multipartFile = request.getFile("fileFromUserFormLicense");
			Long size = multipartFile.getSize();
			String contentType = multipartFile.getContentType();
			InputStream stream = multipartFile.getInputStream();
			File file = ResourceUtils.getFile("classpath:users_license.xml");

//			    File file = new File("users_license.xml");
			List<User> users = ExcelReader.parseExcelFileToBeans(stream, file);
			logger.info("in upload method users size " + users.size());
			if (users.size() == 0) {
				return "No Data Found";
			...
			// "+company.getCompanyName());
			logger.info("Company got in uploadQuestions " + company.getId() + " " + company.getCompanyName());
			String html = FileUtils.readFileToString(new File(propertyConfig.getLmsStudentRegistrationTemplate()));
			for (User u : users) {
				String name = u.getFirstName() == null ? "" : u.getFirstName() + " " + (u.getLastName() == null ? "" : u.getLastName());
				name = name.trim();
				String html1 = html.replace("${FULL_NAME}", name.length() == 0 ? u.getEmail() : name);
				html1 = html1.replace("${EMAIL}", u.getEmail());
				u.setCompanyId(company.getCompanyId());
				u.setCompanyName(company.getCompanyName());
				u.setUserType(UserType.LMS_STUDENT);
				int pwd = u.getEmail().hashCode();
				if (pwd < 0) {
					pwd = pwd * -1;
				}
				u.setPassword(pwd + "");
				html1 = html1.replace("${PASSWORD}", u.getPassword());
				html1 = html1.replace("${COMPANY}", u.getCompanyName());
				userService.saveOrUpdate(u);
				EmailGenericMessageThread messageThread = new EmailGenericMessageThread(u.getEmail(), "Yaksha Credentials", html1, propertyConfig);
	...
	public ModelAndView init(@RequestParam String tenantEmailId, @RequestParam String companyId, @RequestParam String companyName, @RequestParam String pwd,
	...
	public void setUpTenant(@RequestParam String tenantEmailId, @RequestParam String tenantId, @RequestParam String companyName, String adminPassword,
								HttpServletResponse response, HttpServletRequest request) throws Exception {
		 ...
		 * folder
		 */
		FileUtils.copyDirectory(new File(propertyConfig.getDefaultReferenceConfigFileLocation() + File.separator + "assessment"),
									new File(propertyConfig.getTomcatDeployLocation() + File.separator + tenantId));

		/**
		 * Step 4 - Change the appContext file of copied war folder to correspond to
		 * tenant specific values
		 */
		String appContextFileLoc = propertyConfig.getTomcatDeployLocation() + File.separator + tenantId + File.separator + "WEB-INF" + File.separator + "classes"
		...
		String logFile = propertyConfig.getTomcatDeployLocation() + File.separator + tenantId + File.separator + "WEB-INF" + File.separator + "classes" + File.separator
									+ "log4j.properties";
		...
		if (local) {
			message = "Hello \n\n Please go to https://localhost:8443/" + tenantId + "/init?companyId=" + tenantId + "&companyName=" + companyName
			...
			message = "Hello \n\n Please go to " + (propertyConfig.getRemoteBaseUrl() + tenantId) + "/init?companyId=" + tenantId + "&companyName=" + companyName
										+ "&tenantEmailId=" + tenantEmailId + "&pwd=" + pwd + "\n"
										+ "After clicking on above link you can access your instance of Assessment platform by using following - \n<br>"
...
//	        FileUtils.writeByteArrayToFile(dest, bat);
	...
	public @ResponseBody Integer getNonComplianceCount(@RequestBody String userNonCompliance, HttpServletRequest request) throws Exception {
		...
		UserNonCompliance nonCompliance = userNonComplianceService.findNonCompliance(email, testName, companyId);
...
//		    	}
	...
	@RequestMapping(value = "/listLMSUsers", method = RequestMethod.GET)
	public ModelAndView listLMSUsers(@RequestParam(name = "page", required = false) Integer pageNumber, HttpServletResponse response, HttpServletRequest request)
								throws Exception {
		User user = (User) request.getSession().getAttribute("user");
		if (pageNumber == null) {
			pageNumber = 0;
		...
		ModelAndView mav = new ModelAndView("lms_users");
		Page<User> users = userRepository.findLmSUsersCompany(user.getCompanyId(), PageRequest.of(pageNumber, 20));
		mav.addObject("users", users.getContent());

		CommonUtil.setCommonAttributesOfPagination(users, mav.getModelMap(), pageNumber, "listLMSUsers", null);
		return mav;
	}
	...
	public ModelAndView lmsAdmins(@RequestParam(name = "page", required = false) Integer pageNumber, HttpServletResponse response, HttpServletRequest request,
		...
		Page<User> users = userService.findUsersByTypeAndCompany(user.getCompanyId(), UserType.LMS_ADMIN.getType(), PageRequest.of(pageNumber, 15));
	...
	public ModelAndView addlmsadmin(@RequestParam(name = "userId", required = false) Long userId, HttpServletResponse response, HttpServletRequest request) throws Exception {
		User user = (User) request.getSession().getAttribute("user");
	...
	public ModelAndView savelmsadmin(HttpServletRequest request, HttpServletResponse response, @ModelAttribute("usr") User usr, ModelMap modelMap) {
		User user = (User) request.getSession().getAttribute("user");
		usr.setCompanyId(user.getCompanyId());
		usr.setCompanyName(user.getCompanyName());
		usr.setUserType(UserType.LMS_ADMIN);
		userService.saveOrUpdate(usr);
		Page<User> users = userService.findUsersByTypeAndCompany(user.getCompanyId(), UserType.LMS_ADMIN.getType(), PageRequest.of(0, 15));
		ModelAndView mav = new ModelAndView("lmsAdmins");
		mav.addObject("users", users.getContent());
	...
	public @ResponseBody String uploadLMSUsers(HttpServletResponse response, MultipartHttpServletRequest request) throws Exception {
		try {
			// System.out.println("in uploadUsers entering");
			User user = (User) request.getSession().getAttribute("user");
...
//			    File file = new File("users.xml");
			...
			for (User u : users) {
				...
				u.setUserType(UserType.LMS_STUDENT);
				User u1 = userService.findByPrimaryKey(u.getEmail(), company.getCompanyId());
				...
				if (u1 == null) { // first time sent email
					...
					html = html.replace("{FULL_NAME}", u.getFirstName() + " " + u.getLastName());
					...
					EmailGenericMessageThread client = new EmailGenericMessageThread(u.getEmail(),
												"Registration Credentials - " + propertyConfig.getBaseUrl(), html,
												propertyConfig);
	...
	public @ResponseBody String getHTMLTableForClassfier(@RequestParam("classifier") String classifier, HttpServletRequest request, HttpServletResponse response) {
		User user = (User) request.getSession().getAttribute("user");
	...
	public @ResponseBody String shareModule(@RequestParam(required = false, name = "meetingid") String meetingid, @RequestParam("moduleName") String moduleName,
								@RequestParam("classifier") String classifier, HttpServletRequest request, HttpServletResponse response)
								throws IOException {
		...
		String cs = clss[2].trim();
		String companyId = user.getCompanyId();
		List<String> users = userService.findEmailByInstituteGradeClassifier(companyId, institute, grade, cs);
		...
		for (String email : users) {
			LMSUserModuleMapping lmsUserModuleMapping = mappingService.getByPrimaryKey(email, module.getId(), user.getCompanyId());
		...
		EmailGenericMessageThread client = new EmailGenericMessageThread(user.getEmail(), "Sharing Training Module " + module.getModuleName(), html, propertyConfig);
	...
	@ResponseBody
	public String showModuleData(@RequestParam(name = "mname", required = true) String mname, HttpServletRequest request, HttpServletResponse response, ModelMap modelMap)
								throws Exception {
		User user = (User) request.getSession().getAttribute("user");
		...
		container = container.replace("{MODULE_DESC}", module.getModuleDescription() == null ? "NA" : module.getModuleDescription());
		...
		for (ModuleItem item : module.getItems()) {
			...
			UserTestSession session = userTestSessionService.findUserTestSession(user.getEmail(), testName, user.getCompanyId());
			if (session != null) {
		...
		for (ModuleItem item : module.getItems()) {
			temp = rows;
			if (item.getExternalVideoUrl() != null && (item.getExternalVideoUrl().startsWith("http") || item.getExternalVideoUrl().startsWith("www"))) {
			...
			temp = temp.replace("{TEST_NAME}", (test.getTestName().replaceAll("^['\"]*", "").replaceAll("['\"]*$", "")) + "  - " + check);
	...
	@ResponseBody
	public String showModuleDataForPreviewForAdmin(@RequestParam(name = "mname", required = true) String mname, HttpServletRequest request, HttpServletResponse response,
								ModelMap modelMap) throws Exception {
		User user = (User) request.getSession().getAttribute("user");
		Module module = moduleService.findUniqueModule(mname, user.getCompanyId());
	...
	public ModelAndView showUserTests(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) {
		User user = (User) request.getSession().getAttribute("user");
		List<UserTestSession> sessions = userTestSessionService.findTestListForUser(user.getCompanyId(), user.getEmail());
	...
	public ModelAndView showLearenrUserDashboard(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) {
		User user = (User) request.getSession().getAttribute("user");
		List<LMSUserModuleMapping> mappings = mappingService.getAllModulesForUser(user.getCompanyId(), user.getEmail());
		List<ModuleDTO> modules = new ArrayList<ModuleDTO>();
		for (LMSUserModuleMapping mapping : mappings) {
			Module module = moduleService.findUniqueModule(mapping.getModuleName(), user.getCompanyId());
			ModuleDTO moduleDTO = new ModuleDTO();
			moduleDTO.setModule(module);
			moduleDTO.setSharedByEmail(mapping.getCreatedBy() == null ? "NA" : mapping.getCreatedBy());
			User trainer = null;
			if (mapping.getCreatedBy() != null) {
				trainer = userService.findByPrimaryKey(mapping.getCreatedBy(), user.getCompanyId());
				moduleDTO.setSharedByFullName(trainer.getFirstName() + " " + trainer.getLastName());
			}
			SimpleDateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy hh:mm aa");
			Date share = mapping.getUpdateDate() == null ? mapping.getCreateDate() : mapping.getUpdateDate();
			...
			for (ModuleItem item : module.getItems()) {
				String testName = item.getTestName();
				Test test = testService.findbyTest(testName, user.getCompanyId());
				String userTestUrl = getUrlForUser(user.getEmail(), test.getId(), user.getCompanyId());
	...
	public ModelAndView showLearnerAdminDashboard(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) {
		User user = (User) request.getSession().getAttribute("user");
	...
	public String fetchModulesForAdminUserByLicense(@RequestParam(name = "licname", required = true) String licname, HttpServletRequest request, HttpServletResponse response,
		...
		for (Module module : modules) {
			...
			tempMod = tempMod.replace("{MOD_DESC}", module.getModuleDescription() == null ? "NA" : module.getModuleDescription());
			tempModAll += tempMod;
		}
	...
	/**
	 * For Jayramans skins methods
	 */
	@RequestMapping(value = "/fetchModuleItemsForModuleJ", method = RequestMethod.GET)
	@ResponseBody
	public String fetchModuleItemJ(@RequestParam(name = "moduleId", required = true) Long moduleId, HttpServletRequest request, HttpServletResponse response,
								ModelMap modelMap) {
		User user = (User) request.getSession().getAttribute("user");
//		 String modItem = "						<li>                            <div class=\"my-auto\">                                <i class=\"fa fa-desktop\"></i>                            </div>                            <div class=\"module-detail\">                                <h3>${Module_Item_Name}</h3>                                <ul>                                    <li>Test Name: <strong>${Test_Name}</strong></li>                                    <li><a href=\"${Item_Video}\" target=\"_blank\">Click to Open Video</a></li>                                    <li><a href=\"${Test_Link}\" target=\"_blank\">Start Assessment in New Tab</a></li>                                </ul>                            </div>                            <div>                                <button class=\"btn btn-primary start-assessment\" type=\"button\">                                    Start                                </button>                            </div>                        </li>"; 
//		 String mod = "<div class=\"dialog-content\">                <div class=\"module-list\">                    <h2>${Module_Name}</h2>                    <ul class=\"modules\">                        ${Module_Items}                                            </ul>                </div>                <iframe src=\"${Sample_Test}\" frameborder=\"0\"></iframe>            </div>";
		String modItem = "<li>                                                <div class=\"module-detail\">                           <h3>${Module_Item_Name}</h3>                           <ul>                              <li> <i class=\"fa fa-desktop\" aria-hidden=\"true\"></i></li>                              <li>Test Name: <strong>${Test_Name}</strong></li>                              <li><a data-toggle=\"modal\"  data-target=\"#myModal2\" class=\"geturl\" href=\"${Item_Video}\">Click to Open Video</a></li>                               <li><a data-toggle=\"modal\" data-target=\"#myModal1\" class=\"getsiteurl\" href=\"${Test_Link}\">Start Assessment </a></li>                                                                                      </ul>                        </div>                                             </li>";
		String mod = "<div class=\"dialog-content\">               <div class=\"module-list\">                  <a href=\"#\"><i class=\"fa fa-times closeicon\" aria-hidden=\"true\"></i></a>                  <h2>${Module_Name}</h2>                  <ul class=\"modules list mylist\">                     ${Module_Items}                                       </ul>               </div>               <iframe src=\"\" frameborder=\"0\"></iframe>                        </div>";
		...
		for (ModuleItem item : items) {
			String row = modItem;
			row = row.replace("${Module_Item_Name}", item.getItemName());
			String testName = item.getTestName();
			row = row.replace("${Test_Name}", testName);
			String videourl = item.getVideoUrl() == null ? item.getExternalVideoUrl() : item.getVideoUrl();
			if (videourl == null || !videourl.startsWith("http") || !videourl.startsWith("www")) {
				videourl = propertyConfig.getBaseUrl() + "/no_content";
			}
			row = row.replace("${Item_Video}", videourl);
			Test test = testService.findbyTest(testName, user.getCompanyId());
			String testUrl = getUrlForUser(user.getEmail(), test.getId(), user.getCompanyId());
			Date d1 = new Date();
			String sDate = Base64.getEncoder().encodeToString(("" + d1.getTime()).getBytes());
			...
			eDate = URLEncoder.encode(eDate);
			testUrl += "&startDate=" + sDate + "&endDate=" + eDate;
			row = row.replace("${Test_Link}", testUrl);
			rows += row;
			if (sample == null) {
				sample = testUrl;
			}
		...
		mod = mod.replace("${Module_Name}", module.getModuleName());
		// mod = mod.replace("${Sample_Test}", sample);
		mod = mod.replace("${Module_Items}", rows);
		return mod;
	}

	@RequestMapping(value = "/showUserSessions", method = RequestMethod.GET)
	public ModelAndView getUserSessions(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) {
		User user = (User) request.getSession().getAttribute("user");
		ModelAndView mav = new ModelAndView("user_show_sessions");
		List<UserTestSession> sessions = userTestSessionService.findTestListForUser(user.getCompanyId(), user.getEmail());
		mav.addObject("sessions", sessions);
		mav.addObject("user", user);
		return mav;
	...
	@RequestMapping(value = "/learnerDashboardJ", method = RequestMethod.GET)
	public ModelAndView learnerDashboardJ(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mav = null;
		mav = new ModelAndView("dashboard_j");
		User user = (User) request.getSession().getAttribute("user");
		List<LicenseModuleDTO> licModules = loginController.getModulesForUserByLicenses(user);
		List<LicenseModuleDTO> freeLicModules = loginController.getModulesForUserByFreeLicenses(user, user.getCompanyId());
