...
public class TestPdf2 {
	...
	public void testGetQualifiersOutput() throws Exception{
			...
			if(qs.size() >= 25){
				...
				if(partions.size() != 5){
					...
					for(int i=0; i<5; i++){
						...
						FileUtils.write(new File("Drishti.txt"), qsBatch.get(0).getQualifier1()+", "+qsBatch.get(0).getQualifier2()+", "+qsBatch.get(0).getQualifier3().subSequence(0, qsBatch.get(0).getQualifier3().indexOf("."))+", Section "+(i+1)+"\n", true);
...
//		String two = stk.nextToken();
	...
	public void testReadPdf() throws IOException{
		 try {
	            PDDocument doc = PDDocument.load(new File("C:\\del\\drishti_ias\\April  2021 (Part-I).pdf"));
	            PDFTextStripper reader = new PDFTextStripper();
	            reader.setStartPage(2);
	            //reader.setEndPage(i);
	            String text = reader.getText(doc);
